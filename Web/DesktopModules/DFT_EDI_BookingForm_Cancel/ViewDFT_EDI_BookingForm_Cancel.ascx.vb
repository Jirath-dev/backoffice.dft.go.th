...
Namespace NTi.Modules.DFT_EDI_BookingForm_Cancel
    Partial Class ViewDFT_EDI_BookingForm_Cancel
...
#End Region
        ...
        Private Sub rgRequestForm_ItemCreated(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rgRequestForm.ItemCreated
            If TypeOf e.Item Is GridDataItem Then
                Dim deleteLink As HyperLink = DirectCast(e.Item.FindControl("DeleteLink"), HyperLink)
                deleteLink.Attributes("href") = "#"
                deleteLink.Attributes("onclick") = [String].Format("return ShowDeleteForm('{0}');", e.Item.OwnerTableView.DataKeyValues(e.Item.ItemIndex)("invh_run_auto"))
            End If
        End Sub
        ...
        Private Sub rgRequestForm_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rgRequestForm.NeedDataSource
            rgRequestForm.DataSource = LoadAllRequest()
        End Sub

        Function LoadAllRequest() As SqlDataReader
            Try
                objConn = New SqlConnection(strConn)
                myReader = SqlHelper.ExecuteReader(objConn, CommandType.StoredProcedure, "spFormManual_getForCancel", _
                New SqlParameter("@SiteID", lblRoleID.Text))
                Return myReader
