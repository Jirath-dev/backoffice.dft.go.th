...
Namespace NTi.Modules.DFT_EDI_F04_Report_07
    Partial Class ViewDFT_EDI_F04_Report_07
...
#End Region
        Function LoadReport() As SqlDataReader
            Try
                ...
                strCommand = "SELECT A.reference_code2, A.company_name, A.approve_date, " & _
                             ...
                             "WHERE (A.edi_status_id = 'A') AND (A.Rep_status = 'A') AND (B.site_finish = '" & lblRoleID.Text & "') " & _
                             "AND (CONVERT(varchar(8), B.date_finish, 112) BETWEEN " & FunctionUtility.DMY2YMD(rdpFromDate.SelectedDate.Value) & "" & _
                             "AND " & FunctionUtility.DMY2YMD(rdpToDate.SelectedDate.Value) & ")"
        ...
        Protected Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
            Response.Write("<script language ='javascript'> window.open('http://" & DotNetNuke.Common.GetDomainName(Request) & "/DesktopModules/DFT_EDI_F04_Report_07/frmF04_Report_07.aspx?SITEID=" & lblRoleID.Text & "&FROM=" & FunctionUtility.DMY2YMD(rdpFromDate.SelectedDate.Value) & "&TO=" & FunctionUtility.DMY2YMD(rdpToDate.SelectedDate.Value) & "',null,'height=768, width=1024,status=yes, resizable= yes,scrollbars=no, toolbar=no,location=no,menubar=no'); </script>")
        End Sub
    End Class
