...
Module Module_CallBathEng
    ...
    Public Sub checkUpdate_user(ByVal _numINVH_RUN_AUTO As String, ByVal _strUser As String, ByVal _CheckAll As String, ByVal By_CheckCase As Boolean, ByVal By_YearID As String)
        Dim strEDIConn As String = ConfigurationManager.ConnectionStrings("OriginConnection").ConnectionString
        ...
        If _numINVH_RUN_AUTO <> "" Then
            ...
            Select Case By_CheckCase
                Case True
                    Select Case By_YearID
                        Case "1"
                            _md = SqlHelper.ExecuteNonQuery(strEDIConn, CommandType.StoredProcedure, "viCheckUpdateByUserPrint_NewDS_Ole01", New SqlParameter("@INVH_RUN_AUTO", CommonUtility.Get_StringValue(_numINVH_RUN_AUTO)), New SqlParameter("@UserPrintForm", _strUser), New SqlParameter("@CheckAll", _CheckAll))
                        Case "2"
                            _md = SqlHelper.ExecuteNonQuery(strEDIConn, CommandType.StoredProcedure, "viCheckUpdateByUserPrint_NewDS_Ole02", New SqlParameter("@INVH_RUN_AUTO", CommonUtility.Get_StringValue(_numINVH_RUN_AUTO)), New SqlParameter("@UserPrintForm", _strUser), New SqlParameter("@CheckAll", _CheckAll))
                        Case "3"
                            _md = SqlHelper.ExecuteNonQuery(strEDIConn, CommandType.StoredProcedure, "viCheckUpdateByUserPrint_NewDS_Ole03", New SqlParameter("@INVH_RUN_AUTO", CommonUtility.Get_StringValue(_numINVH_RUN_AUTO)), New SqlParameter("@UserPrintForm", _strUser), New SqlParameter("@CheckAll", _CheckAll))
                    End Select
    ...
    'by rut issued date ฟอร์ม ไทย-เปรู เท่านั้นที่ไม่เหมือนฟอร์มอื่น ใช้เช็คแค่ 1 วัน
    Public Function CheckIssuedDate_Forms_TP(ByVal dateEdi As Date, ByVal Check_PrintDate As Date) As Boolean
    ...
    'ByTine 19-10-2558 เพิ่มเติมฟอร์มไทย-ชิลี (TC)
    'ByTine issued date ฟอร์ม ไทย-ชิลี เท่านั้นที่ไม่เหมือนฟอร์มอื่น ใช้เช็คมากกว่า 3 วัน
    ...
    Public Function CheckCount_RVC(ByVal By_invh_run_auto As String, ByVal By_form As String) As Integer
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            Select Case By_form.ToUpper
                ...
                Case "FORM44_4", "FORM44_44", "FORM44_41", "FORM441_4", "FORMD_ESS", "FORMD_ESS_", "FORMAHK"
...
#Region "B2B"
    ''ByTine 10-09-2559
    ''เพิ่มเติม B2B สำหรับ ConvertFormType
    Public Function ConvertFormTypeForB2B(ByVal _FormType As String) As String
        Try
            Dim _FormNameB2B As String = ""
            Dim StrConn As String = ConfigurationManager.ConnectionStrings("OriginConnection").ConnectionString
            Dim npm As New SqlParameter("@form_type", _FormType)
            Dim Strcommand As String = " SELECT B2B_Form_Type FROM form_type WHERE (form_type = @form_type) "
            Dim ds As New DataSet
            ds = SqlHelper.ExecuteDataset(StrConn, CommandType.Text, Strcommand, npm)
            If ds.Tables(0).Rows.Count > 0 Then
                _FormNameB2B = ds.Tables(0).Rows(0).Item("B2B_Form_Type")
            End If
            ...
            '    Case "FORM4_2"
            '        _FormNameB2B = "E"
            '    Case "FORM44_4", "FORM44_41", "FORM44_44", "FORM441_4"
            '        _FormNameB2B = "D"
            '    Case Else
            '        _FormNameB2B = ""
            'End Select
