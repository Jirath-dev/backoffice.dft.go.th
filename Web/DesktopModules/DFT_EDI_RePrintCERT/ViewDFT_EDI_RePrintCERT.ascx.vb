...
#End Region
            ...
            Try
                    ...
                    Case True
                        Select Case DDLYears.SelectedValue
                            Case "1" 'ย้าย 24TradeDBCon มาที่ตัวหลักเนื่องจากการดึงข้อมูลช้า
                                ds = SqlHelper.ExecuteDataset(strEDIConn, CommandType.StoredProcedure, "vi_select_PrintBack_NewDS_01",
                            ...
                            Case "2" 'OriginConnection ต้องเป็น Table อื่น
                                ds = SqlHelper.ExecuteDataset(strEDIConn, CommandType.StoredProcedure, "vi_select_PrintBack_NewDS_02",
                                                                                New SqlParameter("@TCat", TCAT),
                                                                                New SqlParameter("@invh_run_auto", CommonUtility.Get_StringValue(txtSearchValue.Text.Trim())))
                            Case "3" 'OriginConnection ต้องเป็น Table อื่น
                                ds = SqlHelper.ExecuteDataset(strEDIConn, CommandType.StoredProcedure, "vi_select_PrintBack_NewDS_03",
                                                                                New SqlParameter("@TCat", TCAT),
                                                                                New SqlParameter("@invh_run_auto", CommonUtility.Get_StringValue(txtSearchValue.Text.Trim())))
                            Case "4" 'OriginConnection ต้องเป็น Table อื่น
                                ds = SqlHelper.ExecuteDataset(strEDIConn, CommandType.StoredProcedure, "vi_select_PrintBack_NewDS_04",
                                                                                New SqlParameter("@TCat", TCAT),
                                                                                New SqlParameter("@invh_run_auto", CommonUtility.Get_StringValue(txtSearchValue.Text.Trim())))
                            Case "5" 'OriginConnection ต้องเป็น Table อื่น
                                ds = SqlHelper.ExecuteDataset(strEDIConn, CommandType.StoredProcedure, "vi_select_PrintBack_NewDS_05",
                                                                                New SqlParameter("@TCat", TCAT),
                                                                                New SqlParameter("@invh_run_auto", CommonUtility.Get_StringValue(txtSearchValue.Text.Trim())))
                            Case "6" 'OriginConnection ต้องเป็น Table อื่น
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    If rgRequestList.MasterTableView.Items.Count > 0 Then
                        ...
                        If ds.Tables(0).Rows(0).Item("form_type") = "FORM5_2" Or ds.Tables(0).Rows(0).Item("form_type") = "FORMTC_ESS" Then
                            ...
                            Case True
                                ...
                                ElseIf RBListPrinter.Items.Item(0).Selected = True And RBListFormRequest.Items.Item(0).Selected = False Then
                                    ...
                                    Else
                                        ...
                                        'url &= "&form_type=" & run_form_type.ToUpper

                                        'If run_form_type.ToUpper = "FORMAHK" Then
                                        '    url = "/DesktopModules/DFT_EDI_RePrintCERT/frmFormPDF.aspx?INVH_RUN_AUTO=" & runAuto
                                        'End If
                                        ...
                                        Else
                                            ...
                                            If (txtFirstpage.Text = "" And txtLastpage.Text = "") = True Then 'เพื่อ print แบบปกติ
                                                Select Case ReportPrintClass.printMultireport(spl_runForms(i),
                                                                                              ...
                                                                                              "",
                                                                                              "",
                                                                                              txtFirstpage.Text.Trim,
                                                                                              txtLastpage.Text.Trim,
                                                                                              rdoA_ALL.SelectedValue,
                                                                                              rdoD_ALL.SelectedValue,
                                                                                              CommonUtility.Get_StringValue(rdpSelectDatePrint.SelectedDate.Value),
                                                                                              CheckSelectDate.Checked,
                                                                                              check_YearsOle.Checked,
                        ...
                        Case True
                            ...
                            ElseIf RBListPrinter.Items.Item(0).Selected = True And RBListFormRequest.Items.Item(0).Selected = False Then
                                ...
                                Else
                                    ...
                                    Dim url As String = "/DesktopModules/DFT_EDI_CheckAttachment/frmFormPDFDuplicate.aspx?INVH_RUN_AUTO=" & runAuto
                                    If check_YearsOle.Checked Then
                                        url &= "&yearid=" & DDLYears.SelectedValue
                                    End If
                ...
                Select Case check_YearsOle.Checked
                    Case True
                        Select Case DDLYears.SelectedValue
                            ...
                            Case "4"
                                temp_store = "sp_common_UpdateRePrint_NewDS04"
                            Case "5"
        ...
        Private Sub PrintPdf(INVH_RUN_AUTO, site_id)
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                Try
                    Dim file_path As String = ConfigurationManager.AppSettings("CertificateCopyPath")
                    Dim file_name As String = String.Format("{0}\{1}\{2}\{3}.pdf", file_path, ds.Tables(0).Rows(0).Item("approve_year"), ds.Tables(0).Rows(0).Item("form_type"), ds.Tables(0).Rows(0).Item("reference_code2"))
                    Me.streamToPrint = New StreamReader(file_name)
                    Try
                        Me.printFont = New Font("BrowalliaUPC", 12)
                        Dim pd As PrintDocument = New PrintDocument
                        AddHandler pd.PrintPage, AddressOf Me.pd_PrintPage
                        pd.PrinterSettings.PrinterName = ReportPrintClass.GetLaserPrinterName(site_id, ds.Tables(0).Rows(0).Item("form_type"), 1)
                        pd.Print()
                    Finally
        ...
        Private Sub pd_PrintPage(ByVal sender As Object, ByVal ev As PrintPageEventArgs)
            ...
            linesPerPage = (ev.MarginBounds.Height / Me.printFont.GetHeight(ev.Graphics))
            ' Print each line of the file.

            While ((count < linesPerPage) _
                        AndAlso (Not (Me.streamToPrint.ReadLine) Is Nothing))
                yPos = (topMargin _
                            + (count * Me.printFont.GetHeight(ev.Graphics)))
