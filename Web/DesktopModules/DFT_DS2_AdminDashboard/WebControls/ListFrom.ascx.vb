...
Partial Public Class ListFrom
    ...
    Private Sub LoadSite()
        Try
            ...
            strCommand = "SELECT site_id,site_name+' : '+site_id AS site_name  FROM site_plus " &
                         "WHERE (active_status = 'Y') AND (type_site = 'A') AND (ds_status='N') " &
                         "ORDER BY site_code"
            ...
            strCommand = "SELECT site_id,site_name+' : '+site_id AS site_name FROM site_plus " &
                         "WHERE (active_status = 'Y') AND (type_site = 'A') AND (ds_status='Y') " &
                         "ORDER BY site_code"
            ds = SqlHelper.ExecuteDataset(strConn, CommandType.Text, strCommand)
        ...
        Try
            ...
            If status = "N" Then
                If lstSiteOpen.SelectedValue <> "" Then
                    strCommand = "UPDATE site_plus SET ds_status='" & status & "' " &
