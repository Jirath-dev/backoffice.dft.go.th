...
"ORDER BY invh_run_auto"
    ...
    Public Function CheckCount_RVC(ByVal By_invh_run_auto As String, ByVal By_form As String) As Integer
        Dim strEDIConn As String = ConfigurationManager.ConnectionStrings("OriginConnection").ConnectionString
        Dim SQL As String = "SELECT     invh_run_auto, invd_run_auto, tariff_code, letter FROM         form_detail_edi WHERE     (invh_run_auto = @invh_run_auto)"
...
"HAVING        (dbo.form_header_edi.invh_run_auto = @invh_run_auto) AND (dbo.form_detail_edi.CheckGrossDetail = 'GWDetail')"
        ...
        Dim prm As SqlClient.SqlParameter = New SqlClient.SqlParameter("@invh_run_auto", By_invh_run_auto)
        Dim ds As DataSet
        ds = SqlHelper.ExecuteDataset(strEDIConn, CommandType.Text, SQLForm4_8, prm)
    ...
    Public Function Check_DisplayUSD7(ByVal By_invh_run_auto As String) As String
        Dim strEDIConn As String = ConfigurationManager.ConnectionStrings("OriginConnection").ConnectionString
        Dim SQL As String = "SELECT        TOP (1) invh_run_auto, invd_run_auto, DisplayUSD7 " & _
"FROM            form_detail_edi " & _
"WHERE        (invh_run_auto = @invh_run_auto) " & _
"ORDER BY invh_run_auto"
